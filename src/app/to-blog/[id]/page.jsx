...
import classes from './toBlog.module.css'
...
const BlogDetails = (ctx) => {
    ...
    useEffect(() => {
      async function fetchComments(){
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_COMMENT_URL}/api/comment/${ctx.params.id}`, {cache: 'no-store'})
    ...
    useEffect(() => {
        async function fetchBlog() {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BLOG_URL}/api/blog/${ctx.params.id}`, { cache: 'no-store' })
    ...
    const handleDelete = async () => {
        try {
            ...
            if (confirmModal) {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_BLOG_URL}/api/blog/${ctx.params.id}`, {
                    headers: {
    ...
    const handleLike = async () => {
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BLOG_URL}/api/blog/${ctx.params.id}/like`, {
                headers: {
                    'Authorization': `Bearer ${session?.user?.accessToken}`
                },
                method: 'PUT'
    ...
    const handleComment = async () => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_COMMENT_URL}/api/comment`, {
                headers: {
            ...
            <div className={classes.wrapper}>
                ...
                <div className={classes.commentSection}>
                    <div className={classes.commentInput}>
                        ...
                        <button disabled={!session?.user?._id ? true : false} className={!session?.user?._id ? classes.active : classes.btn} onClick={handleComment}>Post</button>
                    </div>
